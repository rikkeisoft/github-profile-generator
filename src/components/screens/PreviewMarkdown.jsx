...
const TitlePreview = ({ prefix, name }) => {
  if (prefix && name) {
    return (
      <h1 className="mb-1 text-center font-semibold text-2xl">{ prefix } { name }</h1>
    )
  }
...
const SubtitlePreview = ({ sub }) => {
  if (sub) {
    return (
      <h3 className="mb-6 text-center">{ sub }</h3>
    )
  }
    ...
    if (link) {
      return (
        <p className="mb-1 font-light tracking-wider">
          { prefix }
          <a href={link} target="_blank" rel="noreferrer" className="font-medium no-underline text-blue-500"> { project }</a>
        </p>
      )
    }
    return (
      <p className="mb-1 font-light tracking-wider">
        { prefix }
        <span href={link} className="font-medium"> { project }</span>
      </p>
    )
  }
  if (prefix && data) {
    return (
      <p className="mb-1 font-light tracking-wider">
        { prefix }
        <span href={link} className="font-medium"> { data }</span>
      </p>
    )
  }
